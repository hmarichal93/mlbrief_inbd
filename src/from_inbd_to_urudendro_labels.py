...
def main(root_dataset = "/data/maestria/datasets/Candice_inbd_1500/",
    ...
    if Path(root_dataset).is_file():
        images_path = [Path(root_dataset)]
    else:
        images_path = Path(f"{root_dataset}/InputImages").rglob("*.jpg") + Path(f"{root_dataset}/InputImages").rglob("*.png") + Path(f"{root_dataset}/InputImages").rglob("*.jpeg")
